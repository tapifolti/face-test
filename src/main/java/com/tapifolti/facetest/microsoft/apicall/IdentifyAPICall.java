...
public class IdentifyAPICall {

    static final String FACEID = "FACEID";
    static final String GROUPID = "GROUPID";
    static String BODY =
            "{\"personGroupId\":\""+GROUPID+"\",\"faceIds\":[\""+FACEID+"\"],\"maxNumOfCandidatesReturned\":2,\"confidenceThreshold\":0.6}";
    ...
    public static String readResponseJson(String jsonResp) {
        ...
        try {
            ...
            if (candidates.length() > 1) {
                System.out.print("1st. personId:" + personId + " confidence:" + confidence);
                candidate = candidates.getJSONObject(1);
                String personId2 = candidate.getString("personId");
                confidence = candidate.getDouble("confidence");

                System.out.println(" 2nd. personId:" + personId2 + " confidence:" + confidence);
            } else {
